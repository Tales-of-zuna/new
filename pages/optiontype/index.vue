        ...
        <v-row>
            <v-col cols="12">
                <v-snackbar v-model="success">
                        ...
                        <div class="d-flex">
                            <v-dialog v-model="addType" width="1000">
                                ...
                                <v-card>
                                    <v-card-title primary-title class="d-flex justify-space-between">
                                        Төрөл нэмэх
                                        <v-btn @click="addType = false" icon><v-icon>mdi-close</v-icon>
                                        </v-btn>
                                    </v-card-title>
                                    <v-card-subtitle>Төрлийн мэдээллийг тусд нь оруулна</v-card-subtitle>

                                    <v-card-text>
                                        <v-row><v-col cols="4">
                                                <v-text-field v-model="optiontype.name" label="Нэр" id="id" dense
                                                    color="orange darken-3" outlined
                                                    hint="Барааны дэлгэрэнгүй нэр"></v-text-field>
                                            </v-col><v-col cols="4"><v-text-field v-model="optiontype.sequence" name="SKU"
                                                    label="Sequence" color="orange darken-3" id="id" dense hint="123456789"
                                                    outlined type="number"></v-text-field></v-col>
                                            <v-btn class="mt-3 ml-5" color="orange" text @click="submitAdd()">
                                                Нэмэх
                                            </v-btn>
                    ...
                    }" :headers="headers" :items="optiontypes" :search="search">
...
export default {
    ...
    methods: {
        ...
        async submitAdd() {
            let response = await this.$axios.post("/v1/option-types/create", { ...this.optiontype, store: "644f5ac6e7fb914f1426e0a1" });
    ...
    async fetch() {
        let response = await this.$axios.post("/v1/option-types", { store: "644f5ac6e7fb914f1426e0a1" });
        if (response.status == 200) {
